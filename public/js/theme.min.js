...
const validators = {
  "data-hs-validation-equal-field": (field) => {
    const equalTarget = document.querySelector(
      field.getAttribute("data-hs-validation-equal-field")
    );
  },
};
    ...
    Array.prototype.slice.call(forms).forEach((form) => {
      for (const validator in validators) {
        Array.prototype.slice
          .call(form.querySelectorAll(`[${validator}]`))
...
HSCore.components.HSList = {
      ...
      if (e.which === 40) {
        ...
        if (!activeItem) {
          ...
          if (activeItem.nextElementSibling) {
            ...
            if (
              newList.list.offsetHeight < newActive.getBoundingClientRect().top
            ) {
              newList.list.scrollTop =
                newActive.getBoundingClientRect().top + newList.list.scrollTop;
            }
          }
        }
      } else if (e.which === 38) {
        ...
        if (!activeItem) {
          ...
          if (activeItem.previousElementSibling) {
            ...
            if (0 > newActive.getBoundingClientRect().top) {
              newList.list.scrollTop =
                newActive.getBoundingClientRect().top +
                newList.list.scrollTop -
                newList.list.offsetHeight;
            }
          }
        }
  ...
  searchMenu: function ($el, settings, newList) {
    const that = this;
    if (
      $el.querySelector(`.${newList.searchClass}`).value.length === 0 ||
      (newList.visibleItems.length === 0 && !settings.empty)
    ) {
  ...
  searchMenuShow: function ($el, settings, newList) {
    ...
    if (!newList.visibleItems.length) {
      var empty = that.helpers.show(
        document.querySelector(settings.empty).cloneNode(true)
      );
